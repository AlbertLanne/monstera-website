import { IMAGES_FICHES, type ImageFiche } from '@/content/fr/fiche-images'
...
export type Hero = { fichier: string; alt: string }
...
export const HEROS_PAGES: Record<string, Hero & { dossier: string }> = {
  'capital-investissement': {
    ...
    alt: 'Immeuble de bureaux contemporain au bord d’une rivière',
  },
  'capital-risque': {
    dossier: 'capital-risque',
    fichier: 'capital-risque-6.webp',
    alt: 'Bâtiment universitaire contemporain aux courbes de béton, éclairé au crépuscule',
  },
  crowdfunding: {
    dossier: 'crowdfunding',
    fichier: 'crowdfunding-10.webp',
    alt: 'Quartier d’affaires suisse à la tombée du jour',
  },
  'developpement-de-projets': {
    dossier: 'developpement-de-projets',
    fichier: 'developpement-de-projets-8.webp',
    alt: 'Tours de logements et de bureaux récemment livrées, en périphérie urbaine',
  },
  'energies-renouvelables': {
    dossier: 'energies-renouvelables',
    fichier: 'energies-renouvelables-2.webp',
    alt: 'Barrage hydroélectrique retenant un lac d’altitude dans les Alpes suisses',
  },
  'financement-immobilier': {
    dossier: 'financement-immobilier',
    fichier: 'financement-immobilier-6.webp',
    alt: 'Vue aérienne d’une vieille ville suisse traversée par une rivière',
  },
  'investissements-start-up': {
    dossier: 'investissements-start-up',
    fichier: 'investissements-start-up-1.webp',
    alt: 'Campus universitaire et parc technologique au bord du Léman',
  },
  'medecine-pharma': {
    dossier: 'medecine-pharma',
    fichier: 'medecine-pharma-3.webp',
    alt: 'Bâle au crépuscule, les tours du quartier pharmaceutique se reflétant dans le Rhin',
  },
  'mezzanine-capital': {
    dossier: 'mezzanine-capital',
    fichier: 'mezzanine-capital-10.webp',
    alt: 'Centre historique d’une ville suisse au crépuscule, le long de la rivière',
  },
  'solutions-technologiques-e-mobilite': {
    ...
    alt: 'Train régional longeant un vignoble en terrasses au bord du Léman',
  },
  ...
  discretion: {
    ...
    alt: 'Volume de béton courbe abritant une esplanade déserte',
  },
  impressum: {
    dossier: 'mezzanine-capital',
    fichier: 'mezzanine-capital-1.webp',
    alt: 'Immeuble de bureaux suisse des années soixante, façade sur rue',
  },
  'mentions-legales': {
    dossier: 'mezzanine-capital',
    fichier: 'mezzanine-capital-7.webp',
    alt: 'Tour de bureaux vitrée se détachant sur un ciel dégagé',
  },
  'politique-de-confidentialite': {
    dossier: 'capital-risque',
    fichier: 'capital-risque-10.webp',
    alt: 'Intérieur clair et vide d’un bâtiment contemporain',
  },
}
...
const CORPS_PAGES: Record<string, Pioche[]> = {
  ...
  // Services : le travail d'évaluation, donc des bureaux et des personnes au travail.
  services: [
    ['mezzanine-capital', 3],
...
export function heroDePage(slug: string): (ImageFiche & { alt: string }) | null {
...
export function imagesDeCorps(slug: string): ImageFiche[] {
  ...
  if (propre) {
    ...
    return propre.filter((i) => !deja.has(i.fichier))
  }
  return (CORPS_PAGES[slug] ?? []).map(trouver).filter((i): i is ImageFiche => i !== null)
}
